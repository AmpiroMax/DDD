...
### 0) Scope update (новый курс проекта)
Проект переориентирован на:
- **Редактор карты (in-game)** — `docs/map_editor.md`
- **Doom-like 2.5D (raycasting)** — `docs/doom_25d.md`

Этот документ остаётся полезным как аудит текущей 2D базы (ECS/loop/координаты/ресурсы/риски), но часть выводов про “террария-подобный” геймплей может стать вторичной по отношению к Editor/2.5D.
